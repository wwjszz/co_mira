...
struct user_data {
  ...
  [[nodiscard]] static task_info *task_info_from_user_data(uint64_t user_data) noexcept {
  ...
  [[nodiscard]] type type_from_user_data() noexcept {
    return type_from_user_data(this->user_data_);
  }

  [[nodiscard]] task_info *task_info_from_user_data() noexcept {
